...
private:
   ...
   //ARP-ip mapping cache
    std::unordered_map<uint32_t, std::pair<EthernetAddress, uint64_t>> arp_cache_{};

    //frame queue and datagram queue
    std::queue<EthernetFrame> frame_queue_{};
    std::queue<std::pair<InternetDatagram, Address>> datagram_queue_{};

    //time and time cache of arp request
    size_t time_since_last_tick_{0};
    std::unordered_map<uint32_t,uint64_t> time_cache_{};
